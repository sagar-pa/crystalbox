...
class SenderMonitorInterval():
    ...
    def as_array(self, features):
        return np.array([self.get(f) / SenderMonitorIntervalMetric.get_by_name(f).scale for f in features], dtype=np.float64)
...
class SenderHistory():
    ...
    def as_array(self):
        ...
        arrays = np.array(arrays, dtype=np.float64)
        return arrays
...
def get_min_obs_vector(feature_names):
    ...
    for feature_name in feature_names:
        ...
        result.append(feature.min_val / feature.scale)
...
def get_max_obs_vector(feature_names):
    ...
    for feature_name in feature_names:
        feature = SenderMonitorIntervalMetric.get_by_name(feature_name)
        result.append(feature.max_val / feature.scale)
    return np.array(result) 
...
def _mi_metric_conn_min_latency(mi):
    ...
    if mi.sender_id in _conn_min_latencies:
...
def _mi_metric_send_ratio(mi):
    ...
    if (thpt > 0.0):
        return min(send_rate / thpt, 1000)
    return 1.0
